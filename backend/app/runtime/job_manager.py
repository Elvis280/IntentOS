...
class Job:
    ...
    def update(
        ...
        *,
        ...
        # Allow passing plan and plan_progress explicitly
        # We fetch it dynamically from kwargs to avoid changing signature everywhere
        # Or add them cleanly here.

    def set_plan(self, plan: dict, progress: dict):
        self.plan = plan
    ...
    def update(
        ...
        *,
        ...
        if world      is not None: self.world      = world
        if history    is not None: self.history    = history
        if plan       is not None: self.plan       = plan
        if plan_progress is not None: self.plan_progress = plan_progress
        if step       is not None:
            self.step     = step
            self.progress = round(step / self.max_steps, 4)
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "plan_progress": getattr(self, "plan_progress", None),
